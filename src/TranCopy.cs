...
namespace TranCopyService{
            public class  TranCopy {
				...
				internal     const  int                            BACKLOG_COPY_MODE				    = 3;  
				...
				foreach (PropertyInfo prop in TranCopyUtilLibrary.tranConfig.GetType().GetProperties())
				{
					...
					if(null!=prop.GetValue(TranCopyUtilLibrary.tranConfig) && null!= prop.Name){
						...
						if(prop.Name.ToString().Trim()=="staging_server" && prop.GetValue(TranCopyUtilLibrary.tranConfig,  null).ToString().Trim()=="localhost" ){
							row["parameter_value"] = "172.25.10.14";
				...
				//reportTableMap.Add("Tran Copy Session Parameters",table); /* Table  removed on Ada's request*/
					...
					try{
						...
						TranCopyUtilLibrary.closeLogFile();
							...
							TranCopyUtilLibrary.writeToLog("Error : " + e.Message);
			 ...
			 if(TranCopyUtilLibrary.showParameterTable)	{
                ...
                public void startTranCopy(){
                   try{ 
						...
						if( serverIsReachable(connectionStringMap["source"])){
							...
							DateTime startParamVal =    startParameterValue;                //long.Parse(startParameterValue.ToString("yyyyMMdd"));
							
							DateTime endParamVal   =     endParameterValue;                         //long.Parse(endParameterValue.ToString("yyyyMMdd"));

							DateTime currentVal    = startParamVal;
                             numberOfTables = 0;
								while(currentVal   >= endParamVal){
									Console.WriteLine("currentVal: "+currentVal.ToString("yyyyMMdd"));
									copyDateMap.Add(numberOfTables, currentVal.ToString("yyyyMMdd"));
									TranCopyUtilLibrary.writeToLog("currentVal: "+currentVal.ToString("yyyyMMdd"));
								    partitionSizeIntervalMap.Add(currentVal.ToString("yyyyMMdd"), currentVal.ToString("yyyyMMdd"));
									currentVal =  currentVal.AddDays(-1);
									++numberOfTables;								
								}
								 break;
								 ...
								 case  RANGE_COPY_MODE:
                                   ...
                                   numberOfTables   = 0;
									    ...
									    startParamVal = startParameterValue;
										currentVal    = startParamVal;
									    endParamVal   = endParameterValue;
										while( currentVal <= endParamVal){
                                                copyDateMap.Add(numberOfTables, currentVal.ToString("yyyyMMdd"));
												partitionSizeIntervalMap.Add(currentVal.ToString("yyyyMMdd"), currentVal.ToString("yyyyMMdd"));
												++numberOfTables;
								 ...
								 break;
								 case BACKLOG_COPY_MODE:
								 string backLogStartDate    =   getAggregateStrFromTable("MIN",TranCopyUtilLibrary.destinationTable, "CONVERT(VARCHAR(8),datetime_req,112)", "destination" );
								startParameterValue         =  string.IsNullOrWhiteSpace(backLogStartDate)?DateTime.Now:DateTime.ParseExact(backLogStartDate, "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture);
							    numOfPrevDays               = TranCopyUtilLibrary.numOfDaysFromStart;
								endParameterValue           = startParameterValue.AddDays((-1*TranCopyUtilLibrary.numOfDaysFromStart));

                            
							 startParamVal = startParameterValue;
							
							 endParamVal   = startParamVal.AddDays(-1* TranCopyUtilLibrary.numOfDaysFromStart);    
						   long.Parse(endParameterValue.ToString("yyyyMMdd"));
                             ...
                             numberOfTables = 0;
								...
								foreach(KeyValuePair<string, string>  param in partitionSizeIntervalMap){
									if (copyScriptStr.Contains("\'"+TranCopyUtilLibrary.copyStartParameter+"\'")){
											startDateStr = param.Key;
											endDateStr   = param.Value;
									}else{
											startDateStr = "\'"+param.Key+"\'";
											endDateStr   = "\'"+param.Value+"\'";

									}
								 string  copySQL	         	= copyScriptStr.Replace(TranCopyUtilLibrary.copyStartParameter,startDateStr);
								 copySQL    	      	        = copySQL.Replace(TranCopyUtilLibrary.copyEndParameter,endDateStr);
							...
							DataTable table = new DataTable("SessionTable");
								...
								copyFilterTransCountMap.Add(i, getAggregateFromTable("MAX",filterTableName, TranCopyUtilLibrary.tranCopyFilterField , "staging" ));
								...
								for (int i=0; i<numberOfTables; i++){
									...
									row["inserted_transaction_count"]  			  = (copyTransCountMap[i] - copyFilterTransCountMap[i])> 0?(copyTransCountMap[i] - copyFilterTransCountMap[i]):0;
									table.Rows.Add(row);
			  ...
			  public static void wait(){
				...
				string finalStagingtable   = createStagingView( );
				StringBuilder insertBuilder  =  new StringBuilder();
				foreach(string  column  in  TranCopyUtilLibrary.destinationTableColumnOrder){
					insertBuilder.Append(column).Append(",");
				}
				insertBuilder.Length--;
				string  insertFields       = insertBuilder.ToString();
				string  selectFields       = "t."+insertFields;
				selectFields			   =  selectFields.Replace(",",",t.");

					string  mergeSql      = TranCopyUtilLibrary.tableMergeScript.Replace("TABLE_INSERT_LIST",insertFields)
																				   .Replace("TEE_PREFIX_INSERT_LIST",selectFields)
																				   .Replace("SOURCE_TABLE",finalStagingtable)
																			       .Replace("DESTINATION_TABLE",TranCopyUtilLibrary.destinationTable);
...
public static void  restartableExecuteScript( string script, string  targetConnectionString){
		    ...
		    public static string getAggregateStrFromTable (string aggr, string tableName,string columnName, string server){
				  
				   string script                       = "SELECT  aggr_val  = "+aggr+"("+columnName+") FROM "+tableName+" WITH (NOLOCK)";
				   DataTable aggregateVal 			   = getDataFromSQL(script, connectionStringMap[server]);
				   return string.IsNullOrEmpty(aggregateVal.Rows[0]["aggr_val"].ToString())?"": aggregateVal.Rows[0]["aggr_val"].ToString();    
...
public static  void  exportAllDataToDestinationTable( ){
			  ...
			  try{
				...
				public static  string  createStagingView( ){

				string   targetConnectionString      = connectionStringMap["destination"];
				StringBuilder viewBuilder         =  new StringBuilder();
				string  tranStagingTablePrefix    = TranCopyUtilLibrary.tranCopyTableNamePrefix+"_"+TranCopyUtilLibrary.tranCopyType.ToLower(); 
				//string  filterTableNamePrefix     = TranCopyUtilLibrary.tranCopyFilterTablePrefix+"_"+TranCopyUtilLibrary.tranCopyType.ToLower(); 
				string  insertScriptTemplate      = "  SELECT  "+TranCopyUtilLibrary.finalSelectFields +"  FROM  [TRANSACTION_TABLE_PLACEHOLDER] WITH (NOLOCK) ";
				string stagingView                = tranStagingTablePrefix+"_"+TranCopyUtilLibrary.tranCopyType+"_view";

				string  dropView                  = " IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].["+stagingView+"]')) BEGIN DROP VIEW [dbo].["+stagingView+"] END ";
				executeScript(dropView, connectionStringMap["destination"]);

				viewBuilder.AppendLine( " CREATE VIEW ["+stagingView+"] AS  ");

				for(int  i=0; i< numberOfTables; i++){
						viewBuilder.AppendLine(insertScriptTemplate.Replace("TRANSACTION_TABLE_PLACEHOLDER", tranStagingTablePrefix+"_"+i.ToString()));
						 viewBuilder.AppendLine("  UNION ALL ");
				}
				for(int  j=0; j<12;  j++)viewBuilder.Length--;

				executeScript(viewBuilder.ToString(), connectionStringMap["destination"]);
...
public  static void sendMailNotification(Dictionary<string, DataTable> dTableMap){
				...
				message.Subject =TranCopyUtilLibrary.emailSubject.Replace("COPY_TYPE",TranCopyUtilLibrary.tranCopyType).Replace("COPY_TIME",DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss"));
			...
			public static void cleanUp(){
                    ...
                    for(int i=0; i<numberOfTables; i++){
					  ...
					  executeScript(stagingTableDeleteScript, connectionStringMap["destination"]);
